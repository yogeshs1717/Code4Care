...
import { Scan, LogIn, UserRound, HeartPulse } from 'lucide-react';
...
export function Header({ variant = 'hero', onSignIn, onOpenProfile, onNewScan, onHome }: HeaderProps) {
  const { user } = useAuth();
        ...
        <div className="flex items-center gap-3">
          ...
          {user && (
            <motion.button
              ...
              className="flex items-center gap-2 rounded-xl border border-amber-200/60 bg-amber-50/80 px-3 py-1.5 text-sm font-medium text-amber-700 hover:bg-amber-100/80 transition-all"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
