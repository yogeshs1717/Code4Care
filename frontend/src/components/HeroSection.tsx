...
export function HeroSection({ onEnterApp }: { onEnterApp: () => void }) {
  ...
  return (
    ...
    >
      {/* ── Sticky viewport ── */}
      <div className="sticky top-0 flex h-dvh w-full items-center justify-center overflow-hidden bg-[#FAFAFA]">
        {/* Subtle background gradient */}
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[#4285F4]/5 via-transparent to-[#34A853]/5" />
        ...
        >
          <motion.h1
            className="bg-gradient-to-br from-[#202124] via-[#4285F4] to-[#34A853] bg-clip-text text-[clamp(3rem,8vw,6rem)] font-extrabold leading-none tracking-tighter text-transparent"
          ...
          <motion.p
            className="mt-4 text-lg text-[#5f6368] sm:text-xl"
        ...
        >
          <motion.div
            className="flex flex-col items-center gap-2 text-sm text-[#5f6368]"
            animate={{ y: [0, 6, 0] }}
        ...
        <div className="relative flex h-[60vh] w-full max-w-2xl items-center justify-center">
          ...
          >
            <div className="relative flex h-48 w-36 flex-col items-center sm:h-56 sm:w-44">
              {/* Jar body */}
              <div className="flex h-full w-full flex-col overflow-hidden rounded-2xl border-2 border-[#e8eaed] bg-white shadow-xl">
                {/* Lid */}
                <div className="h-5 w-full rounded-t-xl bg-[#4285F4]" />
                {/* Label area */}
                <div className="flex flex-1 flex-col items-center justify-center gap-1.5 px-3">
                  <div className="h-2 w-3/4 rounded-full bg-[#e8eaed]" />
                  <div className="h-2 w-1/2 rounded-full bg-[#e8eaed]" />
                  <div className="mt-2 flex items-center gap-1">
                    <div className="h-3 w-3 rounded-full bg-[#34A853]" />
                    <div className="h-2 w-16 rounded-full bg-[#e8eaed]" />
                  </div>
                </div>
              </div>
              {/* Glossy reflection */}
          ...
          >
            <div className="relative flex h-52 w-28 flex-col overflow-hidden rounded-[1.5rem] border-[3px] border-[#202124] bg-white shadow-2xl sm:h-64 sm:w-32">
              {/* Notch */}
              <div className="absolute left-1/2 top-0 z-10 h-4 w-16 -translate-x-1/2 rounded-b-xl bg-[#202124]" />
              {/* Screen content */}
              <div className="flex flex-1 flex-col items-center justify-center gap-2 px-3 pt-4">
                {/* Camera reticle */}
                <div className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-[#4285F4]">
                  <div className="h-2 w-2 rounded-full bg-[#4285F4]" />
                </div>
                <div className="h-1.5 w-3/4 rounded-full bg-[#e8eaed]" />
                <div className="h-1.5 w-1/2 rounded-full bg-[#e8eaed]" />
                {/* "Scan" button */}
                <div className="mt-1 h-3 w-3/4 rounded-full bg-[#4285F4]" />
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            style={{
              ...
              background:
                'linear-gradient(90deg, transparent, #4285F4, #4285F4, transparent)',
              boxShadow: '0 0 12px #4285F4, 0 0 24px #4285F480',
            }}
          />

          {/* ── Enter App Button ── */}
          <motion.button
            className="absolute bottom-0 z-20 rounded-full bg-[#4285F4] px-8 py-3 text-base font-semibold text-white shadow-lg shadow-[#4285F4]/30"
            style={{
